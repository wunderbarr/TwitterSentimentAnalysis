...
def analyze_tweet(tweet):
    ...
    #!!!!emoticons have no replaced
    words=tweet.split()
    result['WORDS']=len(words)
    bigrams=get_bigram(words)
...
def get_bigram_freqdist(bigrams):
    return Counter(bigrams) #Counter itself can be regarded as the form of dict
...
if __name__=='__main__':
    ...
    with open(bi_pkl_file_name, 'wb') as pf:
        pickle.dump(bigram_freq_dist, pf)
    print ('save bi-frequency distribution to %s' % pkl_file_name)
    print ('\n [Analysis Statistics]')
